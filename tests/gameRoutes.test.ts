...
describe('Game Routes', () => {
  ...
  const createTestGame = async (ownerId: number) =>
    models.Game.create({
      name: 'Test Game',
  ...
  it('should add a player to a game', async () => {
    ...
    const playerResponse = await request(app)
      .get(
        `/api/games/${response.body.player.gameId}/players/${response.body.player.id}`
      )
      .expect(200);

    expect(playerResponse.body).to.have.property('id', response.body.player.id);
    expect(playerResponse.body).to.have.property(
      'userId',
      response.body.player.userId
    );
    expect(playerResponse.body).to.have.property(
      'gameId',
      response.body.player.gameId
    );
    expect(playerResponse.body).to.have.property(
      'balance',
      response.body.player.balance
    );
  ...
  it('should not add the same player twice to a game', async () => {
    ...
    expect(secondResponse.body).to.have.property(
      'error',
      `User ${user1.id} is already a player in game ${testGame.id}`
    );
  });
